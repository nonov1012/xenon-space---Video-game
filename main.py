...
sl1 = Lourd(pv_max=500, attaque=300, port_attaque=6, port_deplacement=3, cout=800,
            ...
            image=img_Lourd, tier=4, ligne=8, colonne=8, id=next_uid, path=img_lourd_dir)
next_uid += 1
ships.append(sl1)

sl2 = Lourd(pv_max=500, attaque=300, port_attaque=6, port_deplacement=3, cout=800,
            valeur_mort=800*0.6, taille=(3,3), peut_miner=False, peut_transporter=False,
            image=img_Lourd, tier=4, ligne=12, colonne=12, id=next_uid, path=img_lourd_dir)
next_uid += 1
ships.append(sl2)
...
sm1 = Moyen(pv_max=900, attaque=250, port_attaque=5, port_deplacement=5, cout=800,
            ...
            image=img_moyen, tier=1, ligne=12, colonne=1, id=next_uid, path=img_moyen_dir)
next_uid += 1
ships.append(sm1)
...
st1 = Transport(pv_max=1200, attaque=150, port_attaque=4, port_deplacement=8, cout=800,
            ...
            image=img_transport, tier=1, ligne=7, colonne=7, id=next_uid, path=img_transport_dir)
next_uid += 1
ships.append(st1)

sp1 = Petit(pv_max=300, attaque=100, port_attaque=3, port_deplacement=6, cout=800,
            valeur_mort=800*0.6, taille=(2,2), peut_miner=False, peut_transporter=False,
            image=img_petit, tier=1, ligne=12, colonne=10, id=next_uid, path=img_petit_dir)
next_uid += 1
ships.append(sp1)
...
sf1 = Foreuse(pv_max=500, attaque=0, port_attaque=0, port_deplacement=3, cout=800,
            ...
            image=img_foreuse, tier=1, ligne=16, colonne=16, id=next_uid, path=img_foreuse_dir)
next_uid += 1
ships.append(sf1)

"""
# MotherShip
b2 = MotherShip(fenetre, position=(0, 0), tier=1, largeur=4, hauteur=5, id=next_uid, path=img_base)
next_uid += 1
ships.append(b2)
"""
...
while fonctionne:
    ...
    if selection_ship:  # Si un vaisseau est sélectionné
        if interface_transport_active and selection_cargo:  # Si on est en train de gérer un transport et qu'un cargo est sélectionné
            # Récupérer toutes les positions valides pour débarquer ce cargo
            positions_possibles = selection_ship.positions_debarquement(
                selection_cargo, plateau, NOMBRE_LIGNES, NOMBRE_COLONNES)
            
            # Dessiner un rectangle jaune autour de chaque case valide pour le débarquement
            for ligne, colonne in positions_possibles:
                pygame.draw.rect(
                    fenetre,
                    (255, 255, 0),  # couleur jaune
                    (colonne*TAILLE_CASE, ligne*TAILLE_CASE, TAILLE_CASE, TAILLE_CASE), 3  # épaisseur 3 px
                )
            
            # Si la souris est sur une des positions valides, afficher un aperçu du cargo
            if case_souris in positions_possibles:
                # Récupérer les dimensions du cargo pour redimensionner l'image
                largeur, hauteur = selection_cargo.donner_dimensions(selection_cargo.direction)
                
                # Redimensionner l'image du cargo à la taille de la case(s)
                surf = pygame.transform.scale(
                    selection_cargo.image, (largeur*TAILLE_CASE, hauteur*TAILLE_CASE)
                )
                
                # Rendre l'image semi-transparente pour la preview
                surf.set_alpha(150)
                
                # Afficher la preview à la position de la souris
                fenetre.blit(surf, (case_souris[1]*TAILLE_CASE, case_souris[0]*TAILLE_CASE))
        
        else:  # Sinon, on gère un vaisseau normal (déplacement)
            # Récupérer toutes les positions adjacentes valides pour le déplacement
            ...
            # Dessiner un rectangle jaune autour de chaque case valide pour le déplacement
            for ligne, colonne in positions_possibles:
                pygame.draw.rect(
                    fenetre,
                    (255, 255, 0),  # couleur jaune
            ...
            # Si la souris est sur une des positions valides, afficher un aperçu du vaisseau
            if case_souris in positions_possibles:
                # Récupérer les dimensions du vaisseau selon sa direction
                largeur, hauteur = selection_ship.donner_dimensions(selection_ship.aperçu_direction)
                if selection_ship.direction == "haut":
                    selection_ship.image

                if selection_ship.direction == "droite":
                    selection_ship.image = pygame.transform.rotate(selection_ship.image, -90)

                if selection_ship.direction == "bas":
                    selection_ship.image = pygame.transform.rotate(selection_ship.image, 180)

                if selection_ship.direction == "gauche":
                    selection_ship.image = pygame.transform.rotate(selection_ship.image, 90)

                # Redimensionner l'image du vaisseau pour la preview
                surf = pygame.transform.scale(
                    selection_ship.image, (largeur*TAILLE_CASE, hauteur*TAILLE_CASE)
                )
