...
class IATransport:
    def __init__(self, ship: Transport):
    ...
    def trouver_vaisseaux_proches(self, ships: List[Ship]) -> List[Ship]:
        ...
        transportables = []
        for s in ships:
            if s != self.ship and not s.est_mort() and self.est_transportable(s):
                distance = abs(s.cordonner.x - self.ship.cordonner.x) + abs(s.cordonner.y - self.ship.cordonner.y)
                if distance <= 1:
    ...
    def trouver_vaisseaux_allies(self, ships: List[Ship]) -> List[Ship]:
        ...
        return [s for s in ships if s != self.ship and not s.est_mort() and self.est_transportable(s)]
    ...
    def calculer_position_moyenne(self, vaisseaux: List[Ship]) -> Tuple[int, int]:
        ...
        if not vaisseaux:
            return (self.ship.cordonner.x, self.ship.cordonner.y)
    ...
    def calculer_position_fuite(self, cible: Tuple[int, int], grille: List[List[Point]]) -> Tuple[int, int]:
        ...
        dx = cible[0] - self.ship.cordonner.x
        dy = cible[1] - self.ship.cordonner.y
        ...
        fuite_x = largeur - self.ship.cordonner.x # "Miroir" par rapport à l'axe X
        fuite_y = hauteur - self.ship.cordonner.y # "Miroir" par rapport à l'axe Y
    ...
    def trouver_meilleure_case_adjacente(self, cible: Tuple[int, int], grille: List[List[Point]], ships: List[Ship]) -> Optional[Tuple[int, int]]:
        ...
        cases_possibles = self.ship.positions_possibles_adjacentes(grille)
    ...
    def calculer_priorite_appel(self, vaisseau_appelant: Ship) -> float:
        ...
        return abs(vaisseau_appelant.cordonner.x - self.ship.cordonner.x) + \
               abs(vaisseau_appelant.cordonner.y - self.ship.cordonner.y)
    ...
    def choisir_meilleur_appel(self) -> Optional[Ship]:
        ...
        for vaisseau, destination in self.demandes_transport.items():
            ...
            distance_transport = abs(vaisseau.cordonner.x - self.ship.cordonner.x) + abs(vaisseau.cordonner.y - self.ship.cordonner.y)
    ...
    def jouer_tour(self, grille: List[List[Point]], ships: List[Ship]):
        ...
        if pv_faibles and self.ship.cargaison:
            ...
            for i, vaisseau_embarque in reversed(list(enumerate(self.ship.cargaison))): 
                ...
                if vaisseau_embarque in self.demandes_transport:
                    ...
                    destination = (self.ship.cordonner.x, self.ship.cordonner.y)
        ...
        for vaisseau_embarque in self.ship.cargaison:
            if vaisseau_embarque is None:
                continue
